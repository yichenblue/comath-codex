...
DEFAULT_WORKER_OUTPUT_SCHEMA = "schemas/worker_final_output.schema.json"
...
def archived_workstream_path(workstream_id: str) -> Path | None:
    archive_path = repo_root() / "state" / "archived_workstreams.json"
    if not archive_path.exists():
        return None
    archive = read_json(archive_path)
    for archive_batch in archive.get("archives", []):
        if not isinstance(archive_batch, dict):
            continue
        for item in archive_batch.get("workstreams", []):
            if isinstance(item, dict) and item.get("id") == workstream_id:
                rel_path = item.get("path")
                if not isinstance(rel_path, str):
                    raise ValidationError(f"Archived workstream {workstream_id!r} has invalid path")
                return repo_root() / rel_path
    return None
...
def worker_output_schema(task: dict[str, Any]) -> str:
    value = task.get("output_schema")
    if value is None or value == "":
        return DEFAULT_WORKER_OUTPUT_SCHEMA
    if not isinstance(value, str):
        raise ValidationError("output_schema must be a repo-relative string")
    return value


def structured_output_required(task: dict[str, Any]) -> bool:
    value = task.get("structured_output_required", True)
    if not isinstance(value, bool):
        raise ValidationError("structured_output_required must be a boolean")
    return value
...
def validate_agent_task(task: dict[str, Any], source: Path) -> list[str]:
    ...
    structured_required = task.get("structured_output_required", True)
    if not isinstance(structured_required, bool):
        errors.append(f"{source}: structured_output_required must be a boolean")
    output_schema = task.get("output_schema", DEFAULT_WORKER_OUTPUT_SCHEMA)
    if not isinstance(output_schema, str):
        errors.append(f"{source}: output_schema must be a repo-relative string")
    else:
        try:
            rel_path = ensure_relative_repo_path(output_schema, "output_schema")
        except ValidationError as exc:
            errors.append(f"{source}: {exc}")
        else:
            if structured_required and not (repo_root() / rel_path).exists():
                errors.append(f"{source}: output_schema does not exist: {output_schema}")
    ...
    if isinstance(workstream_id, str) and workstream_id:
        try:
            ...
            try:
                workstream_path = archived_workstream_path(workstream_id)
            except ValidationError as archive_exc:
                errors.append(str(archive_exc))
            if workstream_path is None:
...
def literature_validation_json_gate_errors(
    workstream: Path,
    status: dict[str, Any],
    *,
    relative_path: str,
    ...
    path = artifacts_dir(workstream) / relative_path
    try:
        result = read_json(path)
    except ValidationError as exc:
        return [str(exc)]
    ...
    errors: list[str] = []
    if result.get(pass_field) is not True:
        recorded_errors = result.get("errors", [])
        if isinstance(recorded_errors, list) and recorded_errors:
            for item in recorded_errors:
                errors.append(f"{path}: {item}")
        else:
            errors.append(f"{path}: {description} did not pass")
    if not result.get("generated_at"):
        errors.append(f"{path}: missing generated_at; run {rerun_command}")
    return errors
...
def readiness_errors(
    ...
    errors.extend(
        literature_validation_json_gate_errors(
            workstream,
            status,
            relative_path="search_coverage_validation.json",
            pass_field="passed",
            description="search coverage validation",
            rerun_command="scripts/validate_search_coverage.py",
        )
    )
    errors.extend(
        literature_validation_json_gate_errors(
            workstream,
            status,
            relative_path="theorem_applicability_validation.json",
            pass_field="passed",
            description="theorem applicability validation",
            rerun_command="scripts/validate_theorem_applicability.py",
        )
    )
