...
THEOREM_ENVS = {"theorem", "lemma", "proposition"}
...
def extract_from_tex(path: Path, root: Path, arxiv_id: str) -> list[dict[str, Any]]:
    ...
    begin_re = re.compile(
        r"\\begin\{(?P<env>(?:theorem|lemma|proposition)\*?)\}"
        r"(?:\[(?P<title>(?:[^\]\\]|\\.)*)\])?",
        re.IGNORECASE,
    )
    ...
    for index, match in enumerate(begin_re.finditer(text), start=1):
        ...
        if base_env not in THEOREM_ENVS:
            continue
        ...
        statement_id = f"arxiv:{arxiv_id}:{rel_path}:{base_env}:{index}"
        results.append({
...
def write_markdown(path: Path, statements: list[dict[str, Any]], failures: list[dict[str, str]]) -> None:
    ...
    if failures:
        ...
        lines.append("")
    if not statements:
        lines.append("No theorem, lemma, or proposition environments were extracted.")
    for item in statements:
        lines.extend([
            f"## {item['environment'].title()}",
            "",
