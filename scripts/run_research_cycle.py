...
def literature_objective(args: argparse.Namespace) -> str:
    ...
    parts = [
        ...
        "First run scripts/literature_search.py for the search queries below, "
        "then inspect and summarize the returned sources.",
        "For arXiv results, run scripts/arxiv_source_extract.py to download "
        "available arXiv sources, parse LaTeX, and extract theorem, lemma, and "
        ...
        if literature_id:
            ...
            if not args.no_spawn and not args.no_wait:
                run_review_cycle(
                    ...
                    "not-verified caveats. Confirm that "
                    "artifacts/theorem_statement_verification.json has "
                    "verified=true before approving exact theorem-statement claims.",
                )
